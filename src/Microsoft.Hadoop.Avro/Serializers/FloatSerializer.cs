...
namespace Microsoft.Hadoop.Avro.Serializers
{
    ...
    /// <summary>
    /// Serializes C# float into Avro float.
    /// </summary>
    internal sealed class FloatSerializer : ObjectSerializerBase<FloatSchema>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="FloatSerializer"/> class.
        /// </summary>
        /// <param name="schema">The schema.</param>
        public FloatSerializer(FloatSchema schema) : base(schema)
        {
        }

        protected override Expression BuildSerializerSafe(Expression encoder, Expression value)
        {
            return Expression.Call(encoder, this.Encode<float>(), new Expression[] { Expression.Convert(value, typeof(float)) });
        }
